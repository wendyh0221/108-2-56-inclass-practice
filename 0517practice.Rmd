...
sort(str_sub(char2,1,1)) #sort是排序的意思
...
```{r}
covid19$年齡層
levels(covid19$年齡層) #發現4明明最小卻出現在levels的中間 這是因為在前面orded那邊電腦的判斷錯誤 因為他是根據10位數的大小排序
```
...
###levels重新排列
  ...
  *重新再定義factor一次
   ...
   *levels放第二格時， levels= 可省略。因為電腦會自動判讀
...
#因為population裡面有太多不需要的縣市 像是連江縣等等 所以我們先用covid19裡的縣市存進population
print(population)
```
...
4.完後population第一個元素值最大，它的名稱即是人口最多的縣市，第二元素值次大，它名稱是人口第二多縣市，依此類推。
  我們要的是population排完後的元素名稱。
...
練習:將covid19$縣市 依縣市目前個案數目由大排到小。（hint: 可使用table()來得到個案數統計, 它會自然產生named integer vector）

```{r}
table(covid19$縣市) -> individual
sort(individual,decreasing = T) -> new_individual
new <- names(new_individual)
print(new)
covid19$縣市 <- factor(covid19$縣市,levels=new)
levels(covid19$縣市)
```
...
startingAges <- str_extract(level_names,"^[0-9]+")
print(startingAges) #是一個vector:chr 沒辦法直接sort
```
...
```{r}
levels(factor1) <- c("中部","北部","中部","北部","北部","中部") #levels重複命名就是在做整併的動作
factor1
```
