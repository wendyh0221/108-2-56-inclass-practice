...
練習:創造 可選出來自法商學院的「要/不要」向量，pick_lawBusiness。
...
#因為有的時候法學院可能叫做法律學院 所以可以先用levels來檢查看看到底自己設的是不是正確的
...
練習: 創立 可選出缺失資料的「要/不要」向量pick_na, 並計算有多少筆缺失。
...
```
  ...
  *不可以直接用 `==`的原因是因為 == 是要兩邊引號內完全相同 但原始example裡面 hobby引號裡面是有兩個興趣的
  所以如果這時候用==話 沒有任何一個向量是完全等於("美食") 所以只能用str_detect
...
factor(garbageRoutes$linename) -> garbageRoutes$linename #原本linename是character 要把他成factor
levels(garbageRoutes$linename) #看有幾條路線

  
# 由linename創造： 可篩選出下午路線的「要/不要」向量pick_afternoonRoutes。
pick_afternoonRoutes <- str_detect(garbageRoutes$linename,"下午")
