...
addresses <- c(
  ...
  '金門縣金城鎮賢庵里山前33號',
  '臺中市南屯區工業區二十四路23號1樓')
#這裡遇到的狀況是 我知道號前面一定會有數字 但我沒辦法確每個號前面有多少數字 所以這時候得用+ (一條以上)
```
...
str_view_all(addresses,"[0-9]+") # 樓號也會選到 因為沒有+號
```


###頭尾定位 Anchors

  *如何更準確描述target在整個字串的位置*
...
str_replace(
  ...
  as.character(westernYear) # 要替換的內容 因為replace只針對character所以要先把numeric轉成character
) -> twDate_reformat
...
練習:康熙元年是西元1662年，請將“康熙23年5月6日”轉成date class。（假設月日已是西曆月日）
...
regex_pattern <- glue("(?<={A_regex}){target_regex}(?={B_regex})")
print(regex_pattern)

year <- str_extract(date,regex_pattern)
westernYear <- as.integer(year)+1639

str_replace(date,regex_pattern,
            as.character(westernYear)) -> date_reformat
print(date_reformat)
